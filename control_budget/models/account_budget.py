...
class CrossoveredBudgetLines(models.Model):
    ...
    engage_amount = fields.Monetary(compute='_compute_engage_amount', string='Montant Engage', digits=0)
    available_amount = fields.Monetary(compute='_compute_available_amount', string='Montant Disponible', digits=0)

    engage_amount_db = fields.Monetary(compute="_get_engage", store=True)
    available_amount_db = fields.Monetary(compute="_get_available", store=True)
    ...
    def _compute_engage_amount(self):
        for line in self:
            ...
            if line.analytic_account_id.id:
                self.env.cr.execute("""
                    SELECT SUM(price_subtotal + price_tax)
    ...
    @api.multi
    @api.depends("engage_amount")
    def _get_engage(self):
        for rec in self:
            if rec.engage_amount:
                rec.engage_amount_db = rec.engage_amount
            return rec.engage_amount_db

    @api.multi
    @api.depends("available_amount")
    def _get_available(self):
        for rec in self:
            if rec.available_amount:
                rec.available_amount_db = rec.available_amount
            return rec.available_amount_db
