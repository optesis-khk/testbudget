...
class ControlBudgetReport(models.TransientModel):
    """ Here, we just define class fields.
    For methods, go more bottom at this file.

    The class hierarchy is :
    * ControlBudgetReport
    ** ControlBudgetReportAccount
    *** ControlBudgetReportMoveLine
            For non receivable/payable accounts
            For receivable/payable centralized accounts
    *** ControlBudgetReportPartner
            For receivable/payable and not centralized accounts
    **** ControlBudgetReportMoveLine
            For receivable/payable and not centralized accounts
    """

    _name = 'report_control_budget'
    ...
    account_ids = fields.One2many(
        comodel_name='report_control_budget_account',
...
class ControlBudgetReportAccount(models.TransientModel):

    _name = 'report_control_budget_account'
    _inherit = 'control_budget_abstract'

    report_id = fields.Many2one(
        comodel_name='report_control_budget',
        ondelete='cascade',
        index=True
    )
    ...
    general_budget_id = fields.Many2one('account.budget.post', 'Budgetary Position', index=True)
    analytic_account_id = fields.Many2one('account.analytic.account', 'Analytic Account', index=True)
    planned_amount = fields.Float(digits=(16, 2))
    engage_amount = fields.Float(digits=(16, 2))
    available_amount = fields.Float(digits=(16, 2))
    ...
    move_line_ids = fields.One2many(
        comodel_name='report_control_budget_move_line',
        inverse_name='report_account_id'
    )
    partner_ids = fields.One2many(
        comodel_name='report_control_budget_partner',
        inverse_name='report_account_id'
    )


class ControlBudgetReportPartner(models.TransientModel):

    _name = 'report_control_budget_partner'
    _inherit = 'control_budget_abstract'
...
ORDER BY
    CASE
        WHEN "report_control_budget_partner"."partner_id" IS NOT NULL
        THEN 0
        ELSE 1
    END,
    "report_control_budget_partner"."name"
        """


class ControlBudgetReportMoveLine(models.TransientModel):

    _name = 'report_control_budget_move_line'
    _inherit = 'control_budget_abstract'
    ...
    account_id = fields.Many2one('account.analytic.account', 'Analytic Account', index=True)
    company_id = fields.Many2one(related='account_id.company_id', string='Company', store=True, index=True)
    amount = fields.Float(digits=(16, 2))
    planned_amount = fields.Float(digits=(16, 2))
    available_amount = fields.Float(digits=(16, 2))
    product_id = fields.Many2one('product.product', string='Product', index=True)
    general_account_id = fields.Many2one('account.account', string='Financial Account', index=True)
    ref = fields.Char(string='Ref.')
    partner_id = fields.Many2one('res.partner', string='Fournisseur', store=True, index=True)
...
class ControlBudgetReportCompute(models.TransientModel):
    """ Here, we just define methods.
    For class fields, go more top at this file.
    """

    _inherit = 'report_control_budget'

    @api.multi
    def print_report(self, report_type):
        self.ensure_one()
        if report_type == 'xlsx':
            report_name = 'c_b.report_control_budget_xlsx'
        else:
            _logger.info("----entrer dans fonction-----")
            report_name = 'control_budget.' \
                          'report_control_budget_qweb'
    ...
    def _get_html(self):
        ...
        if report:
            ...
            result['html'] = self.env.ref(
                'control_budget.report_control_budget').render(
                    rcontext)
        return result
    ...
    def _inject_account_values(self):
        """Inject report values for report_control_budget_account."""
...
#             """
    ...
    def _inject_partner_values(self, only_empty_partner=False):
        """ Inject report values for report_control_budget_partner.
...
WITH
    accounts_partners AS
        (
            ...
            FROM
                report_control_budget_account ra
            INNER JOIN
...
AND
    ...
    def _inject_line_not_centralized_values(
        ...
        """ Inject report values for report_control_budget_move_line.
        ...
        WITH
            ...
            query_inject_move_line += """
                    INNER JOIN
                        account_budget_line abl
                            ON ra.general_budget_id = abl.general_budget_id
                    WHERE
                        ra.report_id = %s
                )
                    """
        query_inject_move_line += """
INSERT INTO
    report_control_budget_move_line
    (
        """
        if is_account_line:
            query_inject_move_line += """
...
SELECT
    ...
    NOW() AS create_date,
    abl.date,
    abl.general_budget_id,
    abl.account_id,
    abl.company_id,
    abl.amount,
    abl.planned_amount,
    abl.available_amount,
    abl.product_id,
    abl.general_account_id,
    abl.ref,
    abl.partner_id
        """

        query_inject_move_line += """
FROM report_control_budget_account ra,
        """
#         elif is_partner_line:
#             query_inject_move_line += """
#     report_control_budget_partner rp
# INNER JOIN
#     report_control_budget_account ra ON rp.report_account_id = ra.id
#             """
        query_inject_move_line += """
    account_budget_line abl, account_analytic_account aa
WHERE
    ra.general_budget_id = abl.general_budget_id
        """
#         if self.filter_cost_center_ids:
#             query_inject_move_line += """
# INNER JOIN
#     account_analytic_account aa
#         ON
#             abl.account_id = aa.id
#             """
#         else:
        query_inject_move_line += """
AND   abl.account_id = aa.id
            """
        query_inject_move_line += """
AND
    ra.report_id = %s
...
#             """
        if is_account_line:
            query_inject_move_line += """
ORDER BY
abl.date, abl.general_budget_id
            """
...
#                 report_control_budget_account ra
#             INNER JOIN
#                 account_budget_line abl ON ra.general_budget_id = abl.general_budget_id
#         """
#
#         query_inject_move_line_centralized += """
#             WHERE
...
#         query_inject_move_line_centralized += """
#             GROUP BY
#                 ra.general_budget_id, abl.general_account_id, abl.date, abl.general_budget_id, abl.account_id, abl.company_id, abl.amount, abl.planned_amount, abl.available_amount, abl.product_id, abl.ref, abl.partner_id
#         )
# INSERT INTO
#     report_control_budget_move_line
#     (
...
#     ml.available_amount,
#     ml.product_id,
#     ml.general_account_id,
#     ml.ref,
#     ml.partner_id
# FROM
#     report_control_budget_account ra
# INNER JOIN
#     move_lines ml ON ra.general_budget_id = ml.general_budget_id
# WHERE
#     ra.report_id = %s
#     """
#         query_inject_move_line_centralized += """
    ...
    (
        (
            ...
            FROM
                account_move_line ml
            INNER JOIN
                report_control_budget_move_line rml
...
WHERE
    report_control_budget_move_line.id = tags_values.report_id
            """
        params = {
            'report_id': self.id,
        }
    ...
    def _inject_unaffected_earnings_account_values(self):
        ...
        for report_control_budget_account."""
