...
class ControlBudgetReportWizard(models.TransientModel):
    """Control budget report wizard."""

    _name = "control.budget.report.wizard"
    _description = "Control Budget Report Wizard"
    _inherit = 'control_budget_report_abstract_wizard'
    ...
    def button_export_html(self):
        self.ensure_one()
        action = self.env.ref(
            'control_budget.action_report_control_budget')
        action_data = action.read()[0]
        context1 = action_data.get('context', {})
        if isinstance(context1, pycompat.string_types):
            context1 = safe_eval(context1)
        model = self.env['report_control_budget']
        report = model.create(self._prepare_report_control_budget())
    ...
    def _prepare_report_control_budget(self):
        self.ensure_one()
        return {
